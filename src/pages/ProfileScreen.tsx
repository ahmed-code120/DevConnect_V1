...
export default function ProfileScreen() {
  ...
  const [user, setUser] = useState({
    name: viewUser?.name || currentUser?.name || '',
    handle: viewUser ? (viewUser.handle || ('@' + (viewUser.name || 'User').toLowerCase().replace(/\s+/g, ''))) : (currentUser?.handle || ''),
    bio: viewUser?.bio || currentUser?.bio || '',
    avatar: viewUser?.avatar || currentUser?.avatar || '',
    cover_image: viewUser?.cover_image || currentUser?.cover_image || '',
    location: viewUser?.location || '',
    website: viewUser?.website || '',
    joined: viewUser?.joined || '',
    githubUsername: viewUser?.githubUsername || '',
    twitter_url: viewUser?.twitter_url || '',
    linkedin_url: viewUser?.linkedin_url || '',
    projects: [] as any[]
  });
  // Tab State
  ...
  useEffect(() => {
     if (isOwnProfile && currentUser) {
         ...
         fetchUserProfile(viewUser.id);
         fetchProjects(viewUser.id);
     }
  }, [currentUser, isOwnProfile, viewUser]);

  const fetchUserProfile = async (uid: number) => {
    try {
      const res = await fetch(`/api/php/users.php?id=${uid}`);
  ...
  const fetchProjects = async (uid: number) => {
    try {
      const res = await fetch(`/api/php/projects.php?user_id=${uid}`);
      const result = await res.json();
      if (result.status === 'success') {
        setUser(prev => ({ ...prev, projects: result.data }));
      }
    } catch (e) {
      console.error(e);
    }
  ...
  useEffect(() => {
    if (activeTab === 'Following') {
      const uid = isOwnProfile ? currentUser?.id : user.id;
      if (uid) {
        fetch(`/api/php/follows.php?user_id=${uid}&type=following`)
          .then(res => res.json())
          .then(result => {
            if (result.status === 'success') setFollowingList(result.data);
          })
          .catch(err => console.error(err));
      }
    }
  }, [activeTab, isOwnProfile, currentUser?.id, user.id]);
      ...
      if (result.status === 'success') {
        if (userId === user.id) {
          setUser(prev => ({ 
            ...
            followers: result.data.target_followers
          }));
        }
        ...
        if (activeTab === 'Following') {
           setFollowingList(prev => prev.map(u => u.id === userId ? { ...u, is_following: result.data.is_following } : u));
        }
      }
    } catch (err) { console.error(err); }
  };
  ...
  const handleSaveProfile = async () => {
    if(currentUser && isOwnProfile) {
       try {
           const res = await fetch('/api/php/me.php', {
               method: 'POST',
               headers: { 'Content-Type': 'application/json' },
               credentials: 'include',
  ...
  const handleProfileImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, field: 'avatar' | 'cover') => {
    const file = e.target.files?.[0];
    if (file) {
      const formData = new FormData();
      formData.append('image', file);
      formData.append('type', field === 'avatar' ? 'avatar' : 'cover');

      try {
        const res = await fetch('/api/php/upload.php', {
          method: 'POST',
          body: formData,
        });
        const result = await res.json();
        if (result.status === 'success') {
          ...
          setEditForm(prev => ({ ...prev, [field === 'avatar' ? 'avatar' : 'cover_image']: newUrl }));
          // Also update user state immediately for preview if we are editing
          setUser(prev => ({ ...prev, [field === 'avatar' ? 'avatar' : 'cover_image']: newUrl }));
        } else {
          alert(result.message);
        }
      } catch (err) {
  ...
  const handleAddNewProject = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/php/projects.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          title: newProjectForm.title,
          description: newProjectForm.description,
          github_url: newProjectForm.githubUrl,
  ...
  const confirmDelete = async () => {
    if (projectToDelete) {
      try {
        const res = await fetch(`/api/php/projects.php?id=${projectToDelete}`, { method: 'DELETE' });
        const result = await res.json();
        if (result.status === 'success') {
          setUser(prev => ({
            ...prev,
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto pb-24 md:pb-12">
        <header className={`sticky top-0 z-20 h-16 border-b px-4 md:px-8 flex items-center ${theme === 'dark' ? 'border-[#1f1f1f] bg-[#050505]/80 backdrop-blur-md' : 'border-gray-200 bg-gray-50/80 backdrop-blur-md'}`}>
           ...
           <div>
             ...
             <p className={`text-[11px] ${theme === 'dark' ? 'text-gray-500' : 'text-gray-500'}`}>{user.projects.length} Projects</p>
           </div>
        </header>

        <div className={`relative h-48 md:h-64 w-full group ${!user.cover_image && (theme === 'dark' ? 'bg-[#1a1a1a]' : 'bg-gray-100')}`}>
           {user.cover_image ? (
             <img src={user.cover_image} alt="Cover" className="w-full h-full object-cover transition-all" />
           ) : (
             <div className="w-full h-full flex items-center justify-center">
                <ImageIcon className={`w-12 h-12 ${theme === 'dark' ? 'text-gray-800' : 'text-gray-200'}`} />
             </div>
           )}
        </div>
        ...
        <div className="px-4 md:px-8 relative">
            <div className="absolute -top-16 md:-top-20">
             <div className={`relative rounded-full border-4 ${theme === 'dark' ? 'border-[#050505] bg-[#050505]' : 'border-gray-50 bg-white shadow-xl'} overflow-hidden`}>
                {user.avatar ? (
                  <img src={user.avatar} alt="Avatar" className="w-32 h-32 md:w-40 md:h-40 rounded-full object-cover" />
                ) : (
                  <div className={`w-32 h-32 md:w-40 md:h-40 rounded-full flex items-center justify-center text-4xl font-black ${theme === 'dark' ? 'bg-[#1a1a1a] text-[#FF003C]' : 'bg-gray-100 text-gray-400'}`}>
           ...
           <div className="flex justify-end pt-4 pb-4 h-20">
              {isOwnProfile ? (
                 ...
                 <button 
                   ...
                   className={`flex items-center gap-2 px-6 py-2 rounded-full font-bold text-sm transition-all ${user.is_following ? (theme === 'dark' ? 'bg-[#1f1f1f] text-white hover:bg-[#2a2a2a]' : 'bg-gray-200 text-gray-900 hover:bg-gray-300') : (theme === 'dark' ? 'bg-[#FF003C] text-black shadow-[0_0_15px_rgba(255,0,60,0.3)] hover:bg-[#cc0030]' : 'bg-[#ff2a4b] text-white shadow-lg hover:bg-[#e62643]')}`}
                 >
           ...
           <div className="mt-2 text-left">
              ...
              <div className={`flex flex-wrap items-center gap-x-6 gap-y-2 mt-4 text-xs font-medium ${theme === 'dark' ? 'text-gray-500' : 'text-gray-600'}`}>
                 ...
                 {user.linkedin_url && (
                   <div className="flex items-center gap-1">
                     <LinkIcon className="w-4 h-4" /> <a href={user.linkedin_url.startsWith('http') ? user.linkedin_url : `https://${user.linkedin_url}`} target="_blank" rel="noopener noreferrer" className={`${theme === 'dark' ? 'text-[#FF003C]' : 'text-blue-600'} hover:underline`}>LinkedIn</a>
                   </div>
                 )}
                 {user.twitter_url && (
                   <div className="flex items-center gap-1">
                     <LinkIcon className="w-4 h-4" /> <a href={user.twitter_url.startsWith('http') ? user.twitter_url : `https://twitter.com/${user.twitter_url}`} target="_blank" rel="noopener noreferrer" className={`${theme === 'dark' ? 'text-[#FF003C]' : 'text-blue-600'} hover:underline`}>Twitter</a>
                   </div>
                 )}
                 {user.joined && (
                   <div className="flex items-center gap-1">
                     <Calendar className="w-4 h-4" /> Joined {new Date(user.joined).toLocaleDateString(undefined, { month: 'long', year: 'numeric' })}
                   </div>
                 )}
              </div>
              ...
              <div className="flex items-center gap-6 mt-6">
                 <div className="flex items-center gap-1">
                   <span className={`font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{user.following || 0}</span>
                   <span className={`text-xs ${theme === 'dark' ? 'text-gray-500' : 'text-gray-500'}`}>Following</span>
                 </div>
                 <div className="flex items-center gap-1">
                   <span className={`font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{user.followers || 0}</span>
                   ...
                   >
                     ...
                     <div className={`h-40 relative flex items-center justify-center overflow-hidden ${theme === 'dark' ? 'bg-[#1a1a1a]' : 'bg-gray-100'}`}>
                       {project.image_url ? (
                           ...
                           <div className={`opacity-20 font-mono text-4xl font-black rotate-[-10deg] ${theme === 'dark' ? 'text-[#FF003C]' : 'text-gray-500'}`}>{project.title.substring(0, 3).toUpperCase()}</div>
                       )}
                     ...
                     <div className="p-5">
                       ...
                       <p className={`text-xs mt-1 ${theme === 'dark' ? 'text-gray-500' : 'text-gray-500'} line-clamp-2`}>{project.description}</p>
                       <div className={`flex items-center justify-between mt-4 pt-4 border-t ${theme === 'dark' ? 'border-[#1f1f1f]' : 'border-gray-100'}`}>
                         <div className="flex items-center gap-2">
                           ...
                           <span className={`text-[10px] font-medium ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>Added {new Date(project.created_at).toLocaleDateString()}</span>
                         </div>
                         <a href={project.github_url} onClick={(e) => e.stopPropagation()} target="_blank" rel="noopener noreferrer" className={`opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-[10px] uppercase font-bold tracking-widest ${theme === 'dark' ? 'text-[#FF003C] hover:text-[#cc0030]' : 'text-[#ff2a4b] hover:text-red-700'}`}>View Repo →</a>
                       </div>
                     </div>
                   </div>
                 ))}
           ...
           {activeTab === 'Following' && (
              <div className="mt-8 space-y-4">
                 {followingList.length > 0 ? followingList.map((u) => (
                    <div key={u.id} className={`flex items-center justify-between p-4 rounded-xl border ${theme === 'dark' ? 'bg-[#0f0f0f] border-[#1f1f1f]' : 'bg-white border-gray-100 shadow-sm'}`}>
                       <Link to="/profile" state={{ viewUser: { id: u.id, name: u.full_name, avatar: u.avatar } }} className="flex items-center gap-3">
                          {u.avatar ? <img src={u.avatar} alt="" className="w-12 h-12 rounded-full object-cover" /> : <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold ${theme === 'dark' ? 'bg-[#1a1a1a] text-[#FF003C]' : 'bg-gray-100 text-gray-400'}`}>{u.full_name.charAt(0)}</div>}
                          <div>
                             <h4 className={`font-bold text-sm ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{u.full_name}</h4>
                             <p className="text-[10px] text-gray-500">@{u.handle}</p>
                          </div>
                       </Link>
                       <button 
                         onClick={() => handleToggleFollow(u.id)}
                         className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${u.is_following ? (theme === 'dark' ? 'bg-[#1f1f1f] text-white' : 'bg-gray-100 text-gray-600') : (theme === 'dark' ? 'bg-[#FF003C] text-black' : 'bg-[#ff2a4b] text-white')}`}
                       >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className={`w-full max-w-md p-6 rounded-xl border ${theme === 'dark' ? 'bg-[#0a0a0a] border-[#1f1f1f]' : 'bg-white border-gray-200'} shadow-2xl`}>
             <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setIsNewProjectModalOpen(false)} className={`p-1 rounded-full transition-colors ${theme === 'dark' ? 'hover:bg-[#1f1f1f] text-gray-400' : 'hover:bg-gray-100 text-gray-500'}`}><X className="w-5 h-5" /></button>
             </div>
             ...
             <form onSubmit={handleAddNewProject} className="space-y-4 text-left">
                <div>
                  ...
                  <input type="text" required value={newProjectForm.title} onChange={(e) => setNewProjectForm({...newProjectForm, title: e.target.value})} className={`w-full rounded-lg px-3 py-2 border text-sm transition-colors ${theme === 'dark' ? 'bg-[#121212] border-[#1f1f1f] text-white focus:border-[#FF003C]' : 'bg-gray-50 border-gray-300 text-gray-900 focus:border-[#ff2a4b]'} focus:outline-none`} placeholder="Project Title" />
                </div>
                ...
                <div>
                  ...
                  <textarea required rows={3} value={newProjectForm.description} onChange={(e) => setNewProjectForm({...newProjectForm, description: e.target.value})} className={`w-full rounded-lg px-3 py-2 border text-sm transition-colors ${theme === 'dark' ? 'bg-[#121212] border-[#1f1f1f] text-white focus:border-[#FF003C]' : 'bg-gray-50 border-gray-300 text-gray-900 focus:border-[#ff2a4b]'} focus:outline-none resize-none`} placeholder="Brief description of the project" />
                </div>
                ...
                <div>
                  ...
                  <input type="url" required value={newProjectForm.githubUrl} onChange={(e) => setNewProjectForm({...newProjectForm, githubUrl: e.target.value})} className={`w-full rounded-lg px-3 py-2 border text-sm transition-colors ${theme === 'dark' ? 'bg-[#121212] border-[#1f1f1f] text-white focus:border-[#FF003C]' : 'bg-gray-50 border-gray-300 text-gray-900 focus:border-[#ff2a4b]'} focus:outline-none`} placeholder="https://github.com/..." />
                </div>
                ...
                <div>
                  ...
                  <div 
                    ...
                    className={`relative w-full h-32 rounded-lg border-2 border-dashed flex flex-col items-center justify-center transition-colors cursor-pointer overflow-hidden ${isDragging ? (theme === 'dark' ? 'border-[#FF003C] bg-[#FF003C]/10' : 'border-[#ff2a4b] bg-red-50') : (theme === 'dark' ? 'border-[#1f1f1f] bg-[#121212] hover:border-gray-500' : 'border-gray-300 bg-gray-50 hover:border-gray-400')}`}
                  >
                    <input type="file" accept="image/*" onChange={(e) => { if (e.target.files && e.target.files[0]) { handleImageUpload(e.target.files[0]); } }} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
                    {newProjectForm.image ? (
                        ...
                        <div className="flex flex-col items-center pointer-events-none text-center px-4">
                          ...
                          <p className={`text-xs ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>Click to upload or drag and drop</p>
                        </div>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input type="text" value={newProjectForm.tags} onChange={(e) => setNewProjectForm({...newProjectForm, tags: e.target.value})} className={`w-full rounded-lg px-3 py-2 border text-sm transition-colors ${theme === 'dark' ? 'bg-[#121212] border-[#1f1f1f] text-white focus:border-[#FF003C]' : 'bg-gray-50 border-gray-300 text-gray-900 focus:border-[#ff2a4b]'} focus:outline-none`} placeholder="React, CSS, Node" />
                </div>

                <div className="pt-4 flex gap-3">
                  <button type="button" onClick={() => setIsNewProjectModalOpen(false)} className={`flex-1 py-2 rounded-lg font-bold text-sm border transition-all ${theme === 'dark' ? 'border-[#1f1f1f] text-gray-400 hover:text-white hover:border-gray-500' : 'border-gray-300 text-gray-600 hover:bg-gray-100 hover:border-gray-400'}`}>Cancel</button>
                  <button type="submit" className="flex-1 py-2 rounded-lg font-bold text-sm text-black bg-[#FF003C] hover:bg-[#cc0030] shadow-[0_0_15px_rgba(255,0,60,0.3)] transition-all">Add Project</button>
                </div>
             </form>
          </div>
        </div>
      ...
      {projectToDelete !== null && (
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className={`w-full max-w-sm p-6 rounded-xl border ${theme === 'dark' ? 'bg-[#0a0a0a] border-[#1f1f1f]' : 'bg-white border-gray-200'} shadow-2xl text-center`}>
               <div className="mx-auto w-12 h-12 rounded-full flex items-center justify-center mb-4 bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-500"><Trash2 className="w-6 h-6" /></div>
               <h3 className={`font-bold text-xl mb-2 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Delete Project</h3>
               <p className={`text-sm mb-6 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>Are you sure you want to delete this project?</p>
               <div className="flex gap-3">
               <button onClick={() => setProjectToDelete(null)} className={`flex-1 py-2 rounded-lg font-bold text-sm border transition-all ${theme === 'dark' ? 'border-[#1f1f1f] text-gray-400 hover:text-white hover:border-gray-500' : 'border-gray-300 text-gray-600 hover:bg-gray-100 hover:border-gray-400'}`}>Cancel</button>
               <button onClick={confirmDelete} className="flex-1 py-2 rounded-lg font-bold text-sm text-white bg-red-600 hover:bg-red-700 shadow-md transition-all">Delete</button>
               </div>
            </div>
         </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className={`w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl flex flex-col ${theme === 'dark' ? 'bg-[#121212] border border-[#1f1f1f]' : 'bg-white border border-gray-100'} max-h-[90vh]`}>
            <div className={`p-4 border-b flex items-center justify-between sticky top-0 bg-inherit z-10 ${theme === 'dark' ? 'border-[#1f1f1f]' : 'border-gray-100'}`}>
              <div className="flex items-center gap-3">
                 <button onClick={handleCancelEdit} className={`p-2 -ml-2 rounded-full transition ${theme === 'dark' ? 'hover:bg-[#1f1f1f] text-gray-400' : 'hover:bg-gray-100 text-gray-600'}`}><X className="w-5 h-5" /></button>
                 <h3 className={`font-bold text-lg ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Edit Profile</h3>
              </div>
              <button onClick={handleSaveProfile} className={`font-bold text-sm ${theme === 'dark' ? 'text-[#FF003C] hover:text-[#cc0030]' : 'text-blue-600 hover:text-blue-800'}`}>Done</button>
            </div>
            ...
            <div className="overflow-y-auto p-4 flex-1 hide-scrollbar">
               <div className="mb-6 relative h-32 rounded-xl bg-zinc-800 overflow-hidden group">
                  {editForm.cover_image ? (
                    <img src={editForm.cover_image} alt="Cover Preview" className="w-full h-full object-cover opacity-60" />
                  ) : (
                    <div className="w-full h-full bg-[#1a1a1a] flex items-center justify-center opacity-40">
               ...
               <div className="flex justify-center -mt-16 mb-8">
                  <div className={`relative w-24 h-24 rounded-full border-4 ${theme === 'dark' ? 'border-[#121212] bg-[#1a1a1a]' : 'border-white bg-gray-100'} group overflow-hidden shadow-2xl`}>
                     {editForm.avatar ? (
                       <img src={editForm.avatar} alt="Avatar Preview" className="w-full h-full object-cover opacity-80" />
                     ) : (
                       <div className={`w-full h-full flex items-center justify-center text-2xl font-black ${theme === 'dark' ? 'bg-[#1a1a1a] text-[#FF003C]' : 'bg-gray-100 text-gray-400'}`}>
               ...
               <div className="space-y-5">
                  ...
                  <div className="space-y-1">
                     <label className={`text-xs font-bold uppercase tracking-wider text-gray-500`}>Handle</label>
                  ...
                  <div className="space-y-1">
                     <label className={`text-xs font-bold uppercase tracking-wider text-gray-500`}>GitHub Username</label>
                  ...
                  <div className="space-y-1">
                     <label className={`text-xs font-bold uppercase tracking-wider text-gray-500`}>Twitter URL</label>
                     <input type="text" value={editForm.twitter_url || ''} onChange={(e) => setEditForm({...editForm, twitter_url: e.target.value})} className={`w-full px-0 py-2 border-b bg-transparent focus:outline-none transition-colors ${theme === 'dark' ? 'border-[#2a2a2a] text-white focus:border-[#FF003C]' : 'border-gray-200 text-gray-900 focus:border-[#ff2a4b]'}`} placeholder="https://twitter.com/..." />
                  </div>
                  <div className="space-y-1">
                     <label className={`text-xs font-bold uppercase tracking-wider text-gray-500`}>LinkedIn URL</label>
                     <input type="text" value={editForm.linkedin_url || ''} onChange={(e) => setEditForm({...editForm, linkedin_url: e.target.value})} className={`w-full px-0 py-2 border-b bg-transparent focus:outline-none transition-colors ${theme === 'dark' ? 'border-[#2a2a2a] text-white focus:border-[#FF003C]' : 'border-gray-200 text-gray-900 focus:border-[#ff2a4b]'}`} placeholder="https://linkedin.com/in/..." />
                  </div>
               </div>
            </div>
          </div>
